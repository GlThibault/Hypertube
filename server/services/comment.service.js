...
const addComment = (user, comment, magnet) => {
	db.comments.insert({user: user._id, comment: comment, magnet: magnet, image: user.image_url, name: user.username})
...
const showComment = magnet => {
	return new Promise((resolve, reject) => {
		db.comments.find({ magnet: magnet }).toArray((err, comments) => {
			resolve(comments)
		});
	});
}
